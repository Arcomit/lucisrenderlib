...
public class OBJModel2 {
    ...
    protected final Map<String, OBJModel2> group = new ConcurrentHashMap<>();
    protected int VAO;
    protected int VBO;
    protected int EBO;
    ...
    public void render(String groupName, ResourceLocation texture, PoseStack poseStack, int light, int overlay) {
        if (group.containsKey(groupName)) {
            OBJModel2 model = group.get(groupName);
            ...
            if (group != null) {
                ...
                OBJModel2 model = new OBJModel2(groupObj);
                model.render(texture, poseStack, light, overlay);
    ...
    public void render(ResourceLocation texture, PoseStack poseStack, int light, int overlay) {
        ...
        normalsTextureID = TextureLoader.loadTexture(Lucisrenderlib.prefix("obj/test5_n.png"));
        specularTextureID = TextureLoader.loadTexture(Lucisrenderlib.prefix("obj/test5_s.png"));
        // 测试用
...
//        GL30.glBufferData(GL30.GL_ELEMENT_ARRAY_BUFFER, intBuffer, GL30.GL_STATIC_DRAW);
    ...
    public void cleanup() {
        ...
        group.values().forEach(OBJModel2::cleanup);
        group.clear();
    }
}
